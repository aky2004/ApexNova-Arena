...
const Stat = ({ label, value, sub, accent, icon='✦' }) => (
  <motion.div whileHover={{ y:-4, boxShadow: `0 12px 30px ${accent}25` }} style={{ ...glass, borderRadius:18, padding:'24px', flex:1, minWidth:200, position:'relative', overflow:'hidden', border:`1px solid rgba(255,255,255,0.05)`, borderBottom:`3px solid ${accent}50` }}>
    <div style={{ position:'absolute', top:-15, right:-15, fontSize:80, opacity:0.04, color:accent }}>{icon}</div>
    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
      <div style={{ color:accent, fontSize:14, opacity:0.8 }}>{icon}</div>
      <div style={{ fontSize:10, textTransform:'uppercase', letterSpacing:'0.15em', color:'rgba(255,255,255,0.4)', fontWeight:700 }}>{label}</div>
    </div>
    <div style={{ fontSize:32, fontWeight:800, color:'#fff', fontFamily:font.mono, letterSpacing:'-0.03em', textShadow:`0 0 20px ${accent}30` }}>{value}</div>
      ...
      <div style={{ maxWidth:1100, margin:'0 auto', padding:'24px', position:'relative', zIndex:1 }}>
        ...
        {!loading && !error && data && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} style={{ display:'flex', flexDirection:'column', gap:18 }}>
            ...
            {/* Hero */}
            <motion.div initial={{opacity:0,y:-16}} animate={{opacity:1,y:0}} style={{ ...glass, borderRadius:24, overflow:'hidden', position:'relative', border:'1px solid rgba(255,255,255,0.06)', boxShadow:'0 20px 40px rgba(0,0,0,0.3)' }}>
              <div style={{ position:'absolute', right:0, top:0, bottom:0, width:'50%', background:`radial-gradient(circle at 100% 50%, ${PURPLE}15, transparent 60%)`, pointerEvents:'none' }}/>
              <div style={{ height:4, background:`linear-gradient(90deg,${LIME},${GREEN},${PURPLE})` }}/>
              <div style={{ padding:'36px 40px', display:'flex', gap:32, alignItems:'center', flexWrap:'wrap', position:'relative', zIndex:1 }}>
                <div style={{ position:'relative' }}>
                  <motion.button onClick={()=>avatarRef.current?.click()} whileHover={{scale:1.05, boxShadow:`0 0 30px ${LIME}40`}} title="Change avatar"
                    style={{ width:100, height:100, borderRadius:'50%', border:`3px solid ${LIME}80`, background:'rgba(255,255,255,0.06)', cursor:'pointer', overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center', fontSize:38, fontWeight:800, color:LIME, padding:0, boxShadow:`0 0 20px ${LIME}20` }}>
                    {avatarLoading ? <div style={{ width:32, height:32, borderRadius:'50%', border:`3px solid transparent`, borderTopColor:LIME, animation:'spin 0.8s linear infinite' }}/> :
                  ...
                  <div style={{ position:'absolute', bottom:4, right:4, width:16, height:16, borderRadius:'50%', border:'3px solid #060608', background: user.presence_status==='online' ? GREEN : 'rgba(255,255,255,0.3)' }}/>
                </div>

                <div style={{ flex:1, minWidth:200 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:6, flexWrap:'wrap' }}>
                    <h1 style={{ margin:0, color:'#fff', fontSize:32, fontWeight:800, letterSpacing:'-0.03em' }}>{user.name}</h1>
                    <span style={{ fontSize:11, fontWeight:700, letterSpacing:'0.05em', color: user.presence_status==='online' ? GREEN : 'rgba(255,255,255,0.35)', background: user.presence_status==='online' ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.05)', border:`1px solid ${user.presence_status==='online' ? GREEN+'40' : 'rgba(255,255,255,0.1)'}`, borderRadius:100, padding:'4px 12px', textTransform:'uppercase' }}>● {user.presence_status}</span>
                  </div>
                  <div style={{ color:'rgba(255,255,255,0.4)', fontSize:14, marginBottom:20 }}>
                    {user.gamer_tag ? <span style={{ color:'#fff', fontWeight:600 }}>@{user.gamer_tag}</span> : ''} {user.gamer_tag ? ' · ' : ''}{user.email}
                  </div>
                  <div style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
                    {[{ l:'Global Rank', v: user.rank ? `#${user.rank}` : '—', a:LIME },
                      ...
                      <div key={l} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:12, padding:'10px 18px', display:'flex', flexDirection:'column', gap:4 }}>
                        <div style={{ fontSize:10, color:'rgba(255,255,255,0.35)', textTransform:'uppercase', letterSpacing:'0.12em', fontWeight:600 }}>{l}</div>
                        <div style={{ fontSize:18, fontWeight:800, color:a, fontFamily:font.mono }}>{v}</div>
                      </div>
                    ))}
                  </div>
                </div>

                <motion.button whileHover={{ scale:1.02 }} whileTap={{ scale:0.98 }} onClick={()=>setShowSettings(true)} style={{ background:`linear-gradient(135deg, ${LIME}15, transparent)`, border:`1px solid ${LIME}40`, borderRadius:14, padding:'12px 24px', color:LIME, cursor:'pointer', fontSize:14, fontWeight:700, alignSelf:'flex-start', boxShadow:`0 4px 12px ${LIME}10` }}>⚙ Edit Profile</motion.button>
              </div>
            </motion.div>

            {/* Stats */}
            {stats && (
              <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:0.1}} style={{ display:'flex', gap:16, flexWrap:'wrap' }}>
                <Stat label="Games Played" value={stats.games_played} sub="Snake Championship" accent="#38bdf8" icon="🎮"/>
                <Stat label="Wins" value={stats.wins} sub="All-time" accent={GREEN} icon="🏆"/>
                <Stat label="Win Rate" value={`${stats.win_rate}%`} sub={`${stats.wins}W / ${stats.games_played-stats.wins}L`} accent={stats.win_rate>=50?GREEN:'#f87171'} icon="⚡"/>
                <Stat label="Best Score" value={(stats.best_score||0).toLocaleString()} sub="Personal record" accent={LIME} icon="⭐"/>
                <Stat label="Total Points" value={(user.points||0).toLocaleString()} sub="Live tracking" accent={PURPLE} icon="💎"/>
              </motion.div>
            )}
